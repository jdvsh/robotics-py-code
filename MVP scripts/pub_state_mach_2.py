...
class PendulumPublisher(Node):
    ...
    # qtr_swing_time = 0.3865
    qtr_swing_time = 0.3865 * 3
    ...
    def ext1(self, vel):
        if self.vel_prev_sign != sign(vel):
            ...
            self.end_time = time.time() + self.qtr_swing_time
    ...
    def capdata(self, vel, cur_theta):
        if sign(vel) == self.vel_prev_sign:
            ...
            if abs(self.max_vel) < self.rest_thresh:
                self.state = "rest"
            else:

                y_target = self.arm_len * math.sin(cur_theta) * 0.23
